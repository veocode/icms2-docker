...
class HtmlFormatter
{
	 ...
	 */
	public static function format($html, $indentWith = "\t", $tagsWithoutIndentation = 'html,link,img,meta', $tagsInLine = 'textarea,option')
	{
		...
		$tagsInLine = $tagsInLine ? explode(',', $tagsInLine) : [];
		$lastTag = '';
		...
		foreach ($dom as $index => $element)
		{
			...
			else if ($element['closing'])
			{
				...
				if (in_array($lastTag, $tagsInLine)) {
					$output[] = trim($element['content']);
					continue;
				}
