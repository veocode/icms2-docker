...
Если Docker-образы успели собраться до момента отмены и вы хотите их пересобрать при новой установке (например, чтобы заново создать базу данных с новыми дампами), то сначала удалите эти образы. Выведите список всех образов:
```bash
$ docker images
```
И удалите все, название которых начинается на `icms2-docker_`. Например: 
```bash
