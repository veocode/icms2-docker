<?php $listed = array(); $sep = $this->breadcrumb_separator; ?>
<ul>

    <li class="home">
        <a href="<?php echo $options['home_url']; ?>" title="<?php echo LANG_HOME; ?>"><?php echo LANG_HOME; ?></a>
    </li>

    <?php if ($breadcrumbs) { ?>

        <li class="sep"><?php echo $sep; ?></li>
        ...
        <?php foreach($breadcrumbs as $id=>$item){ ?>
            ...
            <li <?php if (!isset($item['is_last'])){ ?>itemscope itemtype="http://data-vocabulary.org/Breadcrumb"<?php } ?>>
                ...
                <?php } else { ?>
                    <span class="active"><?php html($item['title']); ?></span>
                <?php } ?>
            </li>
